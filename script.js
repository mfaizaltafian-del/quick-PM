...
function projectCard(p, showActions = true) {
  ...
  return `<div class="project-card" onclick="openEditProject('${p.id}')">
    ...
    ${budget > 0 ? `<div class="budget-row">
      <div class="budget-label">Budget${over ? '<span class="overbudget-tag">OVER</span>' : ''}</div>
      <div class="budget-value ${over ? 'budget-over' : 'budget-ok'}">${fmtRp(used)} <span style="color:var(--muted)">/ ${fmtRp(budget)}</span></div>
    </div>` : ''}
  </div>`;
}
...
function openProjectModal() {
  ...
  document.getElementById('proj-budget-used').value = '';
...
function openEditProject(id) {
  ...
  document.getElementById('proj-budget-used').value = p.budgetUsed || '';
...
function saveProject() {
  ...
  const data = {
    name, desc: document.getElementById('proj-desc').value.trim(),
    status: document.getElementById('proj-status').value,
    progress: document.getElementById('proj-progress').value,
    budget: document.getElementById('proj-budget').value,
    budgetUsed: document.getElementById('proj-budget-used').value,
...
function saveTask() {
  ...
  const data = {
    title, projectId: document.getElementById('task-project').value,
